...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `planNo` | String | 排程方案编号 |
| `demandQuantity` | int | 需求数量 |
| `availableCapacity` | int | 可用产能 |
| `materialCoverageRate` | double | 物料齐套率(%) |
| `bottleneckUtilization` | double | 瓶颈利用率(%) |
| `lateOrderCount` | int | 预计延期订单数 |
| `setupChangeCount` | int | 换型次数 |
| `maintenanceConflict` | boolean | 存在维护窗口冲突 |
| `plannerApproved` | boolean | 计划员已确认 |
